...
export default class SignedCounter {
    unsigned = 0;
    signed = 0;
    countingTimeout = null;
    counterProgress = 0;
    static ID_PREFIX = 'twos-comp-';
    constructor() {
        this.registerEvents();
    }
    ...
    registerEvents() {
        let i = 1;
        while (i < 129) {
            let cell = document.getElementById(`${SignedCounter.ID_PREFIX}${i}`);
            if (!cell) {
                return console.error('Unable to get cell for ', i);
            }
            let cellId = i;
            cell.addEventListener('click', (ev) => this.updateValueWith(cellId, (ev.currentTarget).innerHTML !== '1'))
            i <<= 1;
        }
        let unButton = document.getElementById('twos-comp-start-unsigned');
        if (!unButton) {
        ...
        let button = document.getElementById('twos-comp-start-signed');
        if (!button) {
